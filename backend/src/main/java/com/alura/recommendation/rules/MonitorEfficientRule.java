...
@Component
public class MonitorEfficientRule implements RecommendationRule {

    @Override
    public boolean applies(RecommendationRequest request) {
        return request != null
                && ConsumptionCategory.LOW.getModelValue().equalsIgnoreCase(request.category());
    }

    @Override
    public TipKey evaluate(RecommendationRequest request) {
        return TipKey.MONITOR;
    }
}
