...
public class FastApiPredictionClient implements PredictionClient {
    ...
    public PredictionResponse predict(PredictionRequest request) {
        try {
            // Se delega la serialización del DTO tipado directamente a Jackson
            PredictionResponse response = predictionRestClient.post()
                    .uri("/predict")
                    .contentType(MediaType.APPLICATION_JSON)
                    .body(request) // Envío directo del record
                    .retrieve()
                    .body(PredictionResponse.class);

            if (response == null) {
                throw new MlServiceUnavailableException("Respuesta vacía del servicio ML");
            }
            return response;
